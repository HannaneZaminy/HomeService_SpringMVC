...
<form action="${pageContext.request.contextPath}/taskManagement" method="get" id="serviceForm">
    <table class="table table-striped table-success table-hover">
        <tr>
            ...
            <td colspan="2">Work management</td>
        ...
        <c:forEach items="${ordersList}" var="list">
            <tr>
                ...
                <c:if test="${list.situation eq 'Waiting_for_specialist_to_come_Your_place'}">
                    <td rowspan="4">
                        <a onclick="startWork(${list.id});" href="#" id="Start">Start work</a>
                    </td>
                </c:if>
                <c:if test="${list.situation eq 'started'}">
                    <td rowspan="4">
                        <a onclick="endOfWork(${list.id});" href="#" id="endO">work ended</a>
                    </td>
                </c:if>
                <c:if test="${list.situation eq 'done'}">
                    <td rowspan="4">
                        <h2>The work is finished</h2>
                    </td>
                </c:if>
            </tr>
            <tr>
